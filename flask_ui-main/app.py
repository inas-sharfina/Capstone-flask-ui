...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.iloc[:, lambda df: [1,0,3,2]].groupby(by=['Category','App']).sum().sort_values(by='Reviews', ascending=False).head(10).reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
